...
class FrameworkDetector:
    ...
    @classmethod
    def detect_all(cls, search_paths: Optional[List[Path]] = None) -> List[FrameworkInfo]:
    ...
    @classmethod
    def detect_primary(cls, search_paths: Optional[List[Path]] = None) -> Optional[FrameworkInfo]:
...
class FrameworkAdapter(ABC):
    ...
    def __init__(self, config_dir: Path, data_dir: Optional[Path] = None):
        ...
        self._client: Optional[CloudMemoryClient] = None
...
def get_adapter_by_name(name: str) -> Optional[Type[FrameworkAdapter]]:
    """Get adapter class by framework name."""
    return ADAPTER_REGISTRY.get(name)


def get_adapter_for_path(path: Path) -> Optional[FrameworkAdapter]:
